...
module FeriasAgentes.ts {
	...
	export class FeriasAgentesManager {
		...
		public setIndex(v: Index) {
            ...
            v.Organismo.setCallback(function (ui: any) {
        ...
        public diferencia(d1:string, d2:string, id:number) {
        ...
        public controlarFechas(orden, idc, hasta, el) {
		...
		public ajaxPost(v:IControlador, titulo:string): JQueryDeferred {
		 ...
		 public ajaxPostURL(url: string, datapost: any, titulo: string, res?: Resultado): JQueryDeferred {
            var that = this;
            var df = $.Deferred();
            var success = false;
            var retorno: number = 0;
            ...
            $.when($.ajax({
                ...
                success: function (data) {
                    ...
                    if (data[0]) {
                        ...
                        //$("#desde1_" + datapost.agente).attr("disabled", "distabled");
                        //$("#desde2_" + datapost.agente).attr("disabled", "distabled");
                        //$("#desde3_" + datapost.agente).attr("disabled", "distabled");
                        //$("#hasta1_" + datapost.agente).attr("disabled", "distabled");
                        //$("#hasta2_" + datapost.agente).attr("disabled", "distabled");
                        //$("#hasta3_" + datapost.agente).attr("disabled", "distabled");
                    } else {
